...
def check_gou_touch (detection_result):
   ...
   if(len(pose_landmarks_list) >= 2):
        ...
        right_hand_to_right_hand = cal_distance(first_person_right_position, second_person_right_position)
        if(right_hand_to_right_hand <= threshold_distance_of_gou):
            return True
        right_hand_to_left_hand = cal_distance(first_person_right_position, second_person_left_position)
        if(right_hand_to_left_hand <= threshold_distance_of_gou):
            return True
        left_hand_to_right_hand = cal_distance(first_person_left_position, second_person_right_position)
        if(left_hand_to_right_hand <= threshold_distance_of_gou):
            return True
        left_hand_to_left_hand = cal_distance(first_person_left_position, second_person_left_position)
        if(left_hand_to_left_hand <= threshold_distance_of_gou):
            return True
        self_goupon = cal_distance(first_person_left_position,first_person_right_position)
        if(self_goupon <= threshold_distance_of_gou):
            return True
        self_goupon = cal_distance(second_person_left_position, second_person_right_position)
        if(self_goupon <= threshold_distance_of_gou):
            return True
   return False
